...
## ![](../../img/duik/icons/bake.svg){style="width:1em;"} Applying a constraint

As in Blender, a constraint can be *applied*: its result becomes the actual position, rotation or scale of the layer, and the constraint is removed. This works with all the constraints on this page — position, orientation and path — with the [parent constraint](parent.md#parent-constraint), and with the [custom constraints](custom.md): copy location, copy rotation and armature.
...
- If the position, rotation or scale is **animated**, the value is set with a keyframe at the current time, just like when editing an animated value by hand. A constraint which doesn't change the value — no target, a weight or an influence of `0 %` — is simply removed, without adding a keyframe.
- When the last constraint of its kind is applied, its expression is removed too, and the property is free again.

!!! warning
    A copy location, copy rotation or armature constraint is evaluated **alone**, on the unconstrained value of the layer; the other constraints stay in place. Applying the first constraint of a stack keeps the layer where it is, but applying one further down may move it, because the constraints above it now start from its result instead of the other way around. Blender behaves the same way.  
    To apply a whole stack, select all of its effects: they're applied from top to bottom, and the layer doesn't move.

!!! note
    The position, orientation and parent constraints don't stack that way: each one adds its own share to the layer, whatever the others do. Applying any of them keeps the layer where it is, and the ones left keep working on top of its new value.

!!! note
    The [kinematics](kinematics.md) can't be applied: their effects live on the controllers and drive whole chains of other layers. Blender doesn't keep the pose when applying its IK constraint either. [Bake](../automation/tools/bake.md) the chain instead.
